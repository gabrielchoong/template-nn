...
class CML(BaseNetwork):
    ...
    def _build_model(
        ...
        pooling_indices: List[int] | None = None
    ) -> nn.Sequential:
        ...
        return nn.Sequential(
            *self._create_layers(conv_channels, conv_kernel_size, pool_kernel_size, pooling_indices)
        )
    ...
    def _create_layers(
        ...
        pooling_indices_set: Set[int] = set(pooling_indices or [])

        for index, out_size in enumerate(conv_channels[1:]):
            layers += [
                nn.Conv2d(
                    in_channels=in_size,
            ...
            if index in pooling_indices_set:
                layers += [
                    nn.MaxPool2d(
